...
class UnsupportedMethodException(msg : String = "" ) extends ESException(msg)
...
object QueryableMacros{
  ...
  (c: scala.reflect.macros.Context)
  (projection: c.Expr[T => S]): c.Expr[Queryable[S]] = _helper[c.type,S]( c )( "map", projection )
  def flatMap[T:c.WeakTypeTag, S:c.WeakTypeTag]
  (c: scala.reflect.macros.Context)
  (projection: c.Expr[T => Queryable[S]]): c.Expr[Queryable[S]] = _helper[c.type,S]( c )( "flatMap", projection )
  def filter[T:c.WeakTypeTag]
  (c: scala.reflect.macros.Context)
  (projection: c.Expr[T => Boolean]): c.Expr[Queryable[T]] = _helper[c.type,T]( c )( "filter", projection )
  def sortBy[T:c.WeakTypeTag]
  (c: scala.reflect.macros.Context)
  (projection: c.Expr[T => Any]): c.Expr[Queryable[T]] = _helper[c.type,T]( c )( "sortBy", projection )
  def drop[T:c.WeakTypeTag]
  (c: scala.reflect.macros.Context)
  (i: c.Expr[Int]): c.Expr[Queryable[T]] = _helper[c.type,T]( c )( "drop", i )
  def take[T:c.WeakTypeTag]
  (c: scala.reflect.macros.Context)
  (i: c.Expr[Int]): c.Expr[Queryable[T]] = _helper[c.type,T]( c )( "take", i )
}
...
object QueryOps{
  def query[T]( queryable:BaseQueryable[T] ) : QueryOps[T] = common.ES_INTERNAL
}
class QueryOps[T]{
  def map[S]( projection: T => S ) : BaseQueryable[S] = common.ES_INTERNAL
  def flatMap[S]( projection: T => BaseQueryable[S] ) : BaseQueryable[S] = common.ES_INTERNAL
  def filter( projection: T => Boolean ) : BaseQueryable[T] = common.ES_INTERNAL
  def sortBy( projection: T => Any ) : BaseQueryable[T] = common.ES_INTERNAL
  def drop( i:Int ) : BaseQueryable[T] = common.ES_INTERNAL
  def take( i:Int ) : BaseQueryable[T] = common.ES_INTERNAL
  def length[S] : Int = common.ES_INTERNAL
}
